...
@ai_bp.route('/recommendations/<rec_id>', methods=['GET'])
@jwt_required
def detail_rec(rec_id):
    return get_detail(rec_id)

@ai_bp.route('/recommendations/<rec_id>', methods=['DELETE'])
@jwt_required
def delete_rec(rec_id):
    return delete_recommendation(rec_id)

@ai_bp.route('/recommendations/<rec_id>/feedback', methods=['POST'])
@jwt_required
